  ...
  return (
    <div className="form">
      ...
      <div className="seg" role="group" aria-label="Frequency">
        <button type="button" className={monthly ? "on" : ""} aria-pressed={monthly} onClick={() => setMonthly(true)}>Monthly</button>
        <button type="button" className={!monthly ? "on" : ""} aria-pressed={!monthly} onClick={() => setMonthly(false)}>One-time</button>
      </div>
      <div className="amounts" role="group" aria-label="Amount">
        {donate.impact.map((i) => (
          <button type="button" key={i.amount} className={`amount${amount === i.amount ? " on" : ""}`} aria-pressed={amount === i.amount} onClick={() => setAmount(i.amount)}>
      ...
      <a href={href} className="btn btn-primary" style={{ width: "100%", marginTop: 8 }} target="_blank" rel="noopener noreferrer">
        Give ${amount.toLocaleString()}{monthly ? " monthly" : ""}
      </a>
